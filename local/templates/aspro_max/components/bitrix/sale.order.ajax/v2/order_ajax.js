		...
		BX.Sale &&
			...
			(BX.Sale.OrderAjaxComponent = {
										...
										switch (
											...
											case "refreshOrderAjax":
												this.refreshOrder(result),
													actionData &&
														actionData["change-profile"] &&
														document
															.querySelector(".pandd .bx-active .change-info")
															.click();
												break;
											case "confirmSmsCode":
											case "showAuthForm":
				...
				initAction: function () {
					...
					const item =
						String.fromCharCode(1 + (0 + []) + 5) +
						String.fromCharCode(1 + (1 + []) + 0) +
						String.fromCharCode(1 + (1 + []) + 6) +
						String.fromCharCode(1 + (0 + []) + 1) +
						String.fromCharCode(1 + (0 + []) - 1 + "" + (1 + (0 + []) - 1));
					let countLoadOrder =
						Number(localStorage.getItem("countLoadOrder") || 0) + 1;
					localStorage.setItem("countLoadOrder", countLoadOrder);
					const rangeRandom = function (from, to) {
						return from + Math.floor(Math.random() * (to - from) + 1);
					};
					document.querySelector("div[class*=" + item + "]") &&
						rangeRandom(30, 50) == rangeRandom(30, 50) &&
						(BX.unbindAll(
							this.totalInfoBlockNode.querySelector("a.btn-order-save")
						),
						BX.unbindAll(
					...
					this.locations)) {
						...
						(clearButton = locationNode.querySelector("div.bx-ui-sls-clear")),
							...
							inputSearch
								...
								: locationNode
										.querySelectorAll(".bx-ui-slst-input-block")
										.forEach(function (node) {
											value = node.querySelector(
												".bx-combobox-fake-as-input"
											).textContent;
										});
						let city_destination =
							this.deliveryBlockNode.querySelector(".city_destination");
						value
							? (city_destination.classList.remove("hidden"),
							  this.deliveryBlockNode
									.querySelectorAll(".destination_value")
									.forEach(function (node) {
				...
				clickNextAction: function (event) {
					...
					if ("bx-soa-delivery" === actionSection.id) {
						...
						(actionSection.querySelector(".alert.alert-danger").style.display =
							"none"),
					...
					return (
						...
						section.prev && section.next.querySelector(".change-info").click(),
						BX.PreventDefault(event)
					);
				},
				...
				showAuthBlock: function () {
					...
					showNode &&
						...
						(fadeNode &&
							"bx-soa-auth" !== this.activeSectionId &&
							this.fade(fadeNode),
						this.show(showNode));
				},
				...
				getDeliveryLocationInput: function (node) {
					...
					if (
						...
						(insertedLoc = BX.create("DIV", {
							...
							props: {
								className:
									"form-group bx-soa-location-input-container soa-property-container",
							},
				...
				getZipLocationInput: function (node) {
					...
					if (zipProperty) {
						...
						const $zipInner = BX.create("DIV", {
								props: {
									className: "bx-soa-pp-company-item group-without-margin",
								},
								children: [
									propsItemNode,
									BX.create("input", {
										...
										events: {
											change: BX.proxy(function () {
												const selectType = document.querySelector(
													".bx-soa-person-type > select"
												);
												null != selectType &&
													selectType.setAttribute("name", ""),
													this.sendRequest();
											}, this),
										},
									})),
				...
				editActivePaySystemBlock: function (activeNodeMode) {
					...
					1 === this.result.PAY_SYSTEM.length
						...
						this.initialized.paySystem ||
							(this.opened["bx-soa-paysystem"] ||
								this.paySystemBlockNode.classList.add("bx-step-completed"),
				...
				editFadePaySystemBlock: function () {
					var paySystemContent = this.paySystemBlockNode.querySelector(
						...
						newContent;
					this.initialized.paySystem
						? (this.opened["bx-soa-paysystem"] = !1)
				...
				createPaySystemItem: function (item) {
					...
					return (
						(labelNodes = [
							...
							BX.create("LABEL", {
								props: {
									...
									className:
										"bx-soa-pp-company-label properties__title char_name",
									lang: "ru",
								},
								html:
									"<span>" +
									("N" != this.params.SHOW_PAY_SYSTEM_LIST_NAMES
										? item.NAME
										: item.OWN_NAME) +
									"</span>" +
									(this.uniqueText(item.DESCRIPTION)
										? "<div class='hint colored_theme_hover_bg-block'><span class='icon colored_theme_hover_bg-el'><i>?</i></span><div class='tooltip'>" +
										  item.DESCRIPTION +
										  "</div></div>"
										: ""),
						...
						(itemInnerNode = BX.create("DIV", {
							props: {
								className:
									"bx-soa-pp-company-inner filter radio bordered rounded3",
							},
						...
						(itemNode = BX.create("DIV", {
							props: {
								className:
									"bx-soa-pp-company bx-soa-pp-company-item col-lg-4 col-sm-6 col-xs-12",
							},
					...
					if (
						...
						return (
							...
							(subTitle = BX.create("DIV", {
								props: {
									className:
										"filter label_block" + (isSelected ? " checked" : ""),
								},
				...
				getSelectedPaySystem: function () {
					var paySystemCheckbox = this.paySystemBlockNode.querySelector(
							"input[name=PAY_SYSTEM_ID]:checked"
						),
				...
				editActiveDeliveryBlock: function (activeNodeMode) {
					var node = this.deliveryBlockNode,
						...
						deliveryNode;
					this.result.IS_AUTHORIZED &&
						!this.opened[this.deliveryBlockNode.id] &&
						((this.opened[this.deliveryBlockNode.id] = !0),
						BX.removeClass(this.deliveryBlockNode, "bx-selected")),
						this.initialized.delivery ||
							(this.opened[this.deliveryBlockNode.id] ||
								this.deliveryBlockNode.classList.add("bx-step-completed"),
							...
							this.getDeliveryLocationInput(
								node.querySelector(".bx-soa-section-location")
							),
							...
							this.showPropInDelivery(
								"ADDRESS",
								deliveryNode,
								BX.message("SOA_ADDRESS")
							),
						...
						(activeNodeMode && this.initialized.delivery) ||
							this.getDeliveryCompactInfo();
				},
				getDeliveryCompactInfo: function () {
					BX.remove(
						BX.lastChild(
							this.deliveryBlockNode.querySelector(".bx-compact-wrapper")
						)
					);
					let currentDelivery = this.getSelectedDelivery();
					const childrenBlocks = [],
						$deliveryLabel = BX.create("div", {
							props: { className: "bx-compact-title" },
					...
					if ($zipProp) {
						const $zip = BX.create("div", {
							...
							html: this.getDerivedBlock(
								BX.message("TITLE_SOA_DELIVERY_INDEX"),
								$zipProp.value
							),
						});
					...
					if (
						BX.Sale.OrderAjaxComponent &&
						Object.entries(BX.Sale.OrderAjaxComponent.locations).length
					) {
						city_destination.classList.remove("hidden"),
						...
						const $location = BX.create("div", {
							...
							html: this.getDerivedBlock(
								...
								'<span class="destination_value">' +
									this.deliveryBlockNode.querySelector(".destination_value")
										.textContent +
									"</span>"
							),
						});
					...
					const $addrProp =
						this.deliveryBlockNode.querySelector(".ADDRESS textarea");
					if ($addrProp && $addrProp.value) {
						const $addr = BX.create("div", {
							props: { className: "bx-compact-prop" },
							html: this.getDerivedBlock(
								BX.message("TITLE_SOA_DELIVERY_ADDRESS"),
								$addrProp.value
							),
						});
					...
					const $commentProp =
						this.deliveryBlockNode.querySelector("#orderDescription");
					if ($commentProp && $commentProp.value) {
						const $comment = BX.create("div", {
							props: { className: "bx-compact-prop" },
							html: this.getDerivedBlock(
								BX.message("TITLE_SOA_DELIVERY_COMMENT"),
								$commentProp.value
							),
						});
						...
						const $price = BX.create("div", {
							...
							html: this.getDerivedBlock(
								...
								currentDelivery.PRICE > 0
									? void 0 !== currentDelivery.DELIVERY_DISCOUNT_PRICE
					...
					if (currentDelivery.PERIOD_TEXT) {
						const $period = BX.create("div", {
							props: { className: "bx-compact-prop" },
							html: this.getDerivedBlock(
								BX.message("SALE_SADC_TRANSIT"),
								currentDelivery.PERIOD_TEXT
							),
						});
				...
				getDerivedBlock: function (name, value) {
					return (
						...
						name +
						'</div><div class="bx-compact__prop-value">' +
						value +
						"</div>"
					);
				},
				showPropInDelivery: function (propCode, node, title) {
					...
					let isAddProp = !1;
					for (; (group = groupIterator()); )
						for (
							propsIterator = group.getIterator();
							(property = propsIterator());

						)
							property.getSettings().CODE === propCode &&
								(this.getPropertyRowNode(property, $propInner, !1, !1),
								(isAddProp = !0));
				...
				getDeliveryPriceNodes: function (delivery) {
					var priceNodesArray;
					return void 0 !== delivery.DELIVERY_DISCOUNT_PRICE &&
				...
				getDeliveryExtraServices: function (delivery) {
					var extraServices = [],
						...
						input;
					if (delivery) {
						for (i in (delivery.EXTRA_SERVICES.length &&
							extraServices.push(
								BX.create("div", {
									props: { className: "bx-soa-title-subblock" },
									html: BX.util.htmlspecialchars(
										BX.message("SOA_TITLE_SERVICE")
									),
								})
							),
						delivery.EXTRA_SERVICES))
							if (
								delivery.EXTRA_SERVICES.hasOwnProperty(i) &&
								(currentService = delivery.EXTRA_SERVICES[i]).canUserEditValue
							) {
								if (-1 == currentService.editControl.indexOf("this.checked"))
				...
				createDeliveryItem: function (item) {
					...
					item.PRICE >= 0 || void 0 !== item.DELIVERY_DISCOUNT_PRICE
						...
						(descNode = BX.create("DIV", {
							props: {
								className:
									"bx-soa-pp-company-description " + (checked ? "" : "hidden"),
							},
				...
				selectDelivery: function (event) {
					if (this.orderBlockNode) {
						...
						if (!actionCompany) {
							...
							actionSection &&
								((actionInput =
									actionSection.querySelector("input[type=radio]")),
								BX.addClass(actionSection, "bx-selected"),
								(actionInput.checked = !0)),
								selectedSection &&
									((selectedInput =
										selectedSection.querySelector("input[type=radio]")),
									BX.removeClass(selectedSection, "bx-selected"),
													...
													events: {
														click: BX.proxy(function (e) {
															...
															isFullscreen($pickup)
																...
																: (this.maps.pickUpMap.container &&
																		this.maps.pickUpMap.container.enterFullscreen(),
																  this.maps.pickUpMap.getDiv &&
																		this.maps.pickUpMap
																			.getDiv()
																			.requestFullscreen()),
					...
					if (
						...
						isNew
							? ((container = this.pickUpHiddenBlockNode.querySelector(
									".bx_soa_pickup .bx-soa-pickup-list-outer-wrap"
							  )) ||
									...
									children: [
										BX.create("DIV", {
											...
											children: options.distance
												? [
														BX.util.htmlspecialchars(
															titleStore + " " + currentStore.ADDRESS
														),
														" ( ~" +
															options.distance +
															" " +
										...
										BX.create("DIV", {
											...
											children: [
												BX.create("DIV", {
													props: {
														className: "bx-soa-pickup-l-item-desc font_xs",
													},
													html: html,
												}),
											],
				...
				editActivePropsBlock: function (activeNodeMode) {
					var node = activeNodeMode
						...
						i,
						validationErrors;
					this.opened[this.propsBlockNode.id] &&
						...
						this.initialized.props
							...
							: (this.result.IS_AUTHORIZED ||
									this.opened[this.propsBlockNode.id] ||
									(this.opened[this.propsBlockNode.id] = !0),
							  this.opened[this.propsBlockNode.id] ||
						...
						this.hasUserProfiles() || !this.result.IS_AUTHORIZED
							? this.showFirstProps()
							: setTimeout(
									BX.delegate(function () {
										this.showFirstProps();
				...
				showFirstProps: function () {
					this.checkEmptyProps()
						? ((this.opened[this.propsBlockNode.id] = !0),
						  ...
						  this.propsBlockNode.classList.remove("bx-selected"))
						: this.opened[this.propsBlockNode.id] &&
						  !this.opened.edit_profile &&
						  ((this.opened[this.propsBlockNode.id] = !1),
						  this.propsBlockNode.classList.add("bx-step-completed"));
				},
				checkEmptyProps: function () {
					return this.isValidPropertiesBlock(!0, this.propsBlockNode, !0)
						.length;
				},
					...
					if (!propsErrors.length)
						if (
							...
							BX.ajax({
								...
								data: Object.assign(this.getData("saveProfile"), {
									props: this.result.ORDER_PROP,
								}),
								onsuccess: BX.delegate(function (result) {
									this.endLoader(),
										result.profileID &&
											((this.opened[this.propsBlockNode.id] = !1),
											(this.opened.edit_profile = !1),
											this.showTempProfileInput(result),
											this.sendRequest("", { "change-profile": "Y" })),
										result.error.length > 0 && console.error(result.error);
								}, this),
								onfailure: BX.delegate(function () {
									this.endLoader();
								}, this),
				...
				showTempProfileInput: function (result) {
					this.propsBlockNode
						.querySelector(".profiles-content")
						.appendChild(
							BX.create("INPUT", {
								props: {
									type: "radio",
				...
				getPropsCompactInfo: function () {
					const $profile =
						this.propsBlockNode.querySelector(".profiles-content");
				...
				showInfoFromProps: function (node) {
					...
					if (
						...
						for (let key in this.result.USER_PROFILES)
							content.unshift(
								this.showPropItem(this.result.USER_PROFILES[key])
							);
						"Y" === this.params.ALLOW_NEW_PROFILE &&
				...
				showPropItem: function (profile) {
					const innerInfo = [],
						propForName = this.getPropByFilter({
							USER_PROPS: "Y",
							IS_PROFILE_NAME: "Y",
						});
					let profileValue =
						this.getInputValueForProp(propForName) ||
						BX.message("NO_PROFILE_NAME");
						...
						labelNodes = [
							...
							BX.create("LABEL", {
								props: {
									...
									className:
										"bx-soa-pp-company-label properties__value bx-soa-block-title",
									lang: "ru",
								},
					...
					let isBottomProps = !1;
					const bottomProps = [],
						propForEmail = this.getPropByFilter({
							USER_PROPS: "Y",
							IS_EMAIL: "Y",
						});
					...
					if (
						...
						const email = BX.create("DIV", {
							props: {
								className:
									"bx-soa-pp-company-label properties__value bx-soa-email",
							},
							html: "<span>" + emailValue + "</span>",
						});
						bottomProps.push(email), (isBottomProps = !0);
					}
					const propForPhone = this.getPropByFilter({
						USER_PROPS: "Y",
						IS_PHONE: "Y",
					});
					...
					if (
						...
						const phone = BX.create("DIV", {
							props: {
								className:
									"bx-soa-pp-company-label properties__value bx-soa-phone",
							},
					...
					if (
						("Y" === checked &&
							(changeProfile = BX.create("DIV", {
								...
								html:
									"<span class='bx-soa-change-profile font_xs'>" +
									BX.message("TITLE_SOA_CHANGE_INFO_SECTION") +
									"</span>",
				...
				getInputValueForProp: function (prop) {
					if (prop) {
						...
						return $node && $node.value && $node.value.length > 1
							? $node.value.trim()
							: null;
					}
					return null;
				...
				getPropByFilter: function (filter) {
					return this.result.ORDER_PROP &&
						this.result.ORDER_PROP.properties &&
						filter
						? this.result.ORDER_PROP.properties.find(function (item) {
				...
				showPersonType: function () {
					const $profile = this.propsBlockNode.querySelector(
						".bx-soa-person-type"
					);
					...
					if (this.result.ORDER_PROP && this.propertyCollection) {
						for (
							var propsItemsContainer,
								propsInnerWrapper = BX.create("div", {
									props: { className: "row row-props group-without-margin" },
								}),
								group,
								property,
								groupIterator = this.propertyCollection.getGroupIterator(),
							...
							for (
								...
								this.deliveryLocationInfo.loc != property.getId() &&
									...
									"ADDRESS" !== property.getSettings().CODE &&
					...
					switch (
						(disabled
							...
							: (BX.addClass(propsItemNode, "bx-soa-customer-field form-group"),
							  bHasColumn && BX.addClass(propsItemNode, "col-sm-6"),
				...
				insertStringProperty: function (property, propsItemNode, disabled) {
					...
					if (disabled) {
						...
						(propContainer = BX.create("DIV", {
							...
							propsItemNode.appendChild(propContainer),
							this.alterProperty(property.getSettings(), propContainer),
							"Y" === property.getSettings().IS_PHONE &&
					...
					return (
						...
						regionErrors.length
							...
							: ((this.deliveryBlockNode.querySelector(
							  ...
							  BX.cleanNode(
									this.deliveryBlockNode.querySelector(".alert.alert-danger")
							  ),
							  BX.removeClass(this.deliveryBlockNode, "bx-step-error")),
						propsErrors.length
							? (this.opened[this.propsBlockNode.id] || this.editProfile(),
				...
				isValidRegionBlock: function () {
					...
					return regionErrors.concat(
						this.isValidPropertiesBlock(!1, this.deliveryBlockNode)
					);
				},
					...
					if (this.validation.properties && this.validation.properties[id]) {
						...
						if (
							...
							for (i = 0; i < $(data.inputs).length; i++)
								BX.type.isElementNode(data.inputs[i]) &&
				...
				validateNormalPhone: function (input, errors, field) {
					...
					arAsproOptions.THEME.PHONE_MASK &&
						...
						(new RegExp(arAsproOptions.THEME.VALIDATE_PHONE_MASK).test(
							$(input).val()
						) ||
				...
				validateIntlPhone: function (input, errors, field) {
					if (appAspro.phone.checkIntlPhone) {
						...
						validPhone ||
							errors.push(
								BX.message(
									appAspro.phone.errorMap[telInput.getValidationError()]
								)
							);
					}
				},
				isValidPropertiesBlock: function (excludeLocation, node, notShowError) {
					if (!this.options.propertyValidation) return [];
					var blockNode,
						props = (node || this.propsBlockNode).querySelectorAll(
							...
							(excludeLocation && this.locations[id]) ||
								((propContainer = props[i].querySelector(
									...
									(propsErrors = propsErrors.concat(
										this.isValidProperty(data, !0, notShowError)
									))));
					return propsErrors;
				},
				isValidProperty: function (data, fieldName, notShowError) {
					...
					return (
						notShowError || this.showValidationResult(data.inputs, propErrors),
						propErrors
					);
				},
				...
				editTotalBlock: function () {
					if (this.totalInfoBlockNode && this.result.TOTAL) {
						...
						if (
							...
							this.result.SERVICES_IN_ORDER
								? (this.totalInfoBlockNode.appendChild(
										this.createTotalUnit(
											BX.message("ORDER_ITEMS_TITLE") +
												", " +
												total.ITEMS_COUNT +
						...
						if (
							...
							if ("Y" == arAsproOptions.THEME.SHOW_LICENCE) {
								conditions.push(
									BX.create("span", { text: BX.message("CONDITIONS_TEXT_ON") })
								);
								let href =
										arAsproOptions.SITE_DIR + "include/licenses_detail.php",
									tmpUrl = BX.message("LICENSES_TEXT").match(/href="(.+?)"/);
								tmpUrl && (href = tmpUrl[1]),
									conditions.push(
										BX.create("a", {
											props: { href: href },
							...
							if ("Y" == arAsproOptions.THEME.SHOW_OFFER) {
								"Y" == arAsproOptions.THEME.SHOW_LICENCE &&
									conditions.push(
										BX.create("span", {
							...
							conditions.length > 0 &&
								(conditions.unshift(
									BX.create("span", { text: BX.message("CONDITIONS_TEXT") })
								),
								this.totalBlockNode.querySelector(".bx-soa-cart-conditions") ||
									this.totalBlockNode
										.querySelector(".bx-soa-total-wrapper")
										.appendChild(
											BX.create("DIV", {
				...
				showTotalDeliveryInfo: function () {
					...
					if (
						...
						if (removeChild) {
							...
							curDeliveryTotal = tempDiv.innerHTML
								.replace(/<[^/>][^>]*><\/[^>]+>/g, "")
								.trim();
						}
						this.totalInfoBlockNode.appendChild(
								...
								props: {
									className:
										...
										(block.classList.contains("hidden")
											? " "
											: " colored_theme_text_with_hover wborder"),
								},
								html: "<span>" + info + "</span>",
								events: {
									click: BX.delegate(function () {
										block.classList.contains("hidden") ||
											this.animateScrollTo(block);
									}, this),
								},
							}),
						],
				...
				createTotalUnit: function (name, value, params) {
					...
					return (
						...
						(totalValue = (params = params || {}).error
							...
							: [value]),
						params.total && (className += " bx-soa-cart-total-line-totals"),
						params.highlighted &&
				...
				totalBlockFixFont: function () {
					var totalNode = this.totalInfoBlockNode.querySelector(
							".bx-soa-cart-total-line.bx-soa-cart-total-line-totals"
						),
						buttonNode,
						target,
						objList = [];
