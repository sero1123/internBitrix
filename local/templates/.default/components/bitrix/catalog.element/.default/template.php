	...
	<div class="container-fluid">
		...
		<?
							...
							foreach ($similarProducts as $product)
							{
								...
								?>
									<button onclick = "replacementItem(<?=$product?>)"><?=$color . "<br>"?></button>
								<?;
							}
							?>
						</div>
...
<script>
	function replacementItem(idElem){
		site = 'https://uchenik-test.6hi.ru/catalog/elektronika/telefony/'+idElem+'/?bitrix_include_areas=Y&clear_cache=Y'
		console.log(site);
		$.ajax({
    url: "/testkomponent.php",         /* Куда отправить запрос */
    method: 'get',             /* Метод запроса (post или get) */
    dataType: 'html',          /* Тип данных в ответе (xml, json, script, html). */
    data: {'ELEMENT_ID': idElem},     /* Данные передаваемые в массиве */
    success: function(data){   /* функция которая будет выполнена после успешного запроса.  */
		$('#myBodyTest').empty();
		window.history.pushState("object or string", "Title", site); 
