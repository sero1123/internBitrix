...
class SaleOrderAjax extends \CBitrixComponent
{
	...
	protected function getInnerPaySystemInfo(Order $order, $recalculate = false)
	{
		...
		if ($this->arParams['PAY_FROM_ACCOUNT'] === 'Y' && $order->isAllowPay())
		{
			...
			$percent = $this->arParams['PERCENT_DISCOUNT'];

			// Проверяем, что значение процента находится в диапазоне от 1 до 100
			if ($percent > 100) {
				$percent = 100;
			} elseif ($percent < 0) {
				$percent = 0;
			...
			// Вычисляем ограничение суммы расходов
			$maxSpending = $order->getPrice() * ($percent / 100);

			// Проверяем, превышает ли сумма расходов ограничение
			if ($sumToSpend > $maxSpending) {
				$sumToSpend = $maxSpending;
			}
